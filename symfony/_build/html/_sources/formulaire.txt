...
Symfony dispose du composant ``Form`` pour faciliter la création des formulaires. Ce composant permet de créer un formulaire à partir d'un objet existant, et son objectif est d'hydrater cet objet.
...
Lorsque l'on affiche le formulaire, Symfony choisit le champ HTML qui sera le plus adapté à l'attribut : le titre est une ligne de texte, la date est un ensemble de 5 listes déroulantes (jour, mois, années, heure, minute), le boby est un textarea et les tags, une sélection multiple.
...
Certains types peuvent prendre des paramètres. Pour plus d'informations voici la page de documentation de Symfony : http://symfony.com/fr/doc/current/reference/forms/types.html

**********************
Validation des données
**********************

Le composant ``Form`` de Symfony permet de créer facilement des formulaires et d'hydrater l'objet qui lui est associé, mais il ne valide pas les données du formulaire tout simplement car ce n'est pas à lui le faire. Pour rappel, dans un modèle MVC, le seul et unique responsable des données est le modèle, l'entité.

Premièrement voici le namespace à utiliser dans les annotations pour ajouter des contraintes : ``use Symfony\Component\Validator\Constraints as Assert;``
...
Pour plus d'informations voici la page de documentation de Symfony : http://symfony.com/fr/doc/current/reference/constraints.html

*******************************
Personnalisation de l'affichage
*******************************

Depuis sa version 2.6, Symfony intègre deux layouts basés sur Bootstrap. Nous allons utilisé le layout ``bootstrap_3_horizontal``.
